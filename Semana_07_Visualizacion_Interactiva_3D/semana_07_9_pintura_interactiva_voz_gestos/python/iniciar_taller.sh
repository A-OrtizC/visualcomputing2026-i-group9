...
# 4. Crear el script de Python con la solución del taller
cat << 'EOF' > main.py
import cv2
...
# 5. Volver a la raíz del proyecto para crear la carpeta media y el README base
cd ..
mkdir -p media
...
## Descripción
Este proyecto utiliza OpenCV y MediaPipe para detectar gestos de las manos a través de la webcam y ejecutar acciones visuales.

## Implementaciones (Python)
- **Detección de dedos:** Cuenta cuántos dedos están extendidos.
- **Distancia:** Mide la distancia entre la punta del pulgar y el índice, modificando el tamaño de un objeto.
- **Acciones Visuales:** - Cambia el color de la pantalla mostrando de 1 a 4 dedos.
  - Cambia de escena (de video en vivo a una interfaz opaca) al mostrar la palma abierta (5 dedos).
  - Un objeto en pantalla sigue la posición del dedo índice.

## Resultados Visuales
*(Añade aquí las imágenes o GIFs dentro de la carpeta media/)*
- `![Prueba 1](media/captura1.png)`
- `![Prueba 2](media/captura2.png)`

## Aprendizajes y Dificultades
*(Escribe tu reflexión personal)*
EOF

# 6. Regresar a python y ejecutar la aplicación
cd python
python main.py
